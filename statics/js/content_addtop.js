...
function thumb_images(iframeWin,returnid) {
...
function change_images(iframeWin,returnid){
	var in_content = iframeWin.$("#att-status").html().substring(1);
	var in_filename = iframeWin.$("#att-name").html().substring(1);
...
function change_multifile(iframeWin,returnid){
	var in_content = iframeWin.$("#att-status").html().substring(1);
	var in_filename = iframeWin.$("#att-name").html().substring(1);
	var str = '';
	var contents = in_content.split('|');
...
$.getJSON("?app=content&controller=content&action=public_getjson_ids&modelid="+modelid+"&id="+id, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有添加相关文章');
		return false;
