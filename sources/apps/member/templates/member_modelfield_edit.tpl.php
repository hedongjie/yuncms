...
$(function(){
	...
	$("#field")
		...
		.regexValidator({
			...
			onerror:"<?php echo L('filedname_incorrect')?>"
		})
		.inputValidator({
			min:1,
			max:20,
		...
		.ajaxValidator({
	    	type : "get",
			url : "?app=member&controller=member_modelfield&action=public_checkfield&modelid=<?php echo $modelid?>&oldfield=<?php echo $field;?>",
			data : "",
			datatype : "html",
			cached:false,
			async:'true',
	...
	$("#formtype")
		.formValidator({
			...
			onfocus:"<?php echo L('filedtype_need')?>",oncorrect:"<?php echo L('input_correct')?>",
			defaultvalue:""
		})
		.inputValidator({
			min:1,
			onerror: "<?php echo L('filedtype_need')?>"
		});
	$("#name")
		.formValidator({
		...
		.inputValidator({
			min:1,
			onerror:"<?php echo L('filed_nickname_need')?>"
		});
})
...
<form name="myform" id="myform" action="?app=member&controller=member_modelfield&action=edit" method="post">
