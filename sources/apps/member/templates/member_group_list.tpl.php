...
?>
<form name="myform" id="myform" action="<?php echo U('member/member_group/delete');?>" method="post" onsubmit="check();return false;">
<div class="pad-lr-10">
...
?>
    <tr>
		...
		<td align="center"><?php echo $v['total']?></th>
...
<input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?app=member&controller=member_group&action=sort'" value="<?php echo L('sort')?>"/>
</div>
...
function edit(id, name) {
	window.top.art.dialog.open('?app=member&controller=member_group&action=edit&groupid='+id,{
		title:'<?php echo L('edit').L('member_group')?>《'+name+'》',
		id:'edit',
		width:'700px',
		height:'500px',
...
function check() {
	if(myform.action == '?app=member&controller=member_group&action=delete') {
		var ids='';
