...
$(function(){
	...
	$("#name")
		.formValidator({
			onshow:"<?php echo L('input').L('groupname')?>",
			onfocus:"<?php echo L('groupname').L('between_2_to_8')?>"
		})
		.inputValidator({
			min:2,
			max:15,
			onerror:"<?php echo L('groupname').L('between_2_to_8')?>"
		})
		.regexValidator({
			regexp:"username",
			datatype:"enum",
			onerror:"<?php echo L('groupname').L('format_incorrect')?>"
		})
		.ajaxValidator({
	    	type : "get",
			url : "?app=member&controller=member_group&action=public_checkname_ajax",
			data :"",
			datatype : "html",
			async:'false',
	...
	$("#group_point")
		.formValidator({
			tipid:"pointtip",
			onshow:"<?php echo L('input').L('point')?>",
			onfocus:"<?php echo L('point').L('between_1_to_8_num')?>"
		})
		.regexValidator({
			regexp:"^\\d{1,8}$",
			onerror:"<?php echo L('point').L('between_1_to_8_num')?>"
		});
	$("#group_starnum")
		.formValidator({
			tipid:"starnumtip",
			onshow:"<?php echo L('input').L('starnum')?>",
			onfocus:"<?php echo L('starnum').L('between_1_to_8_num')?>"
		})
		.regexValidator({
			regexp:"^\\d{1,8}$",
			onerror:"<?php echo L('starnum').L('between_1_to_8_num')?>"
		});
	$("#maxmessagenum")
		.formValidator({
			tipid:"maxmessagenumtip",
			onshow:"<?php echo L('input').L('maxmessagenum')?>",
			onfocus:"<?php echo L('maxmessagenum').L('between_1_to_8_num')?>"
		})
		.regexValidator({
			regexp:"^\\d{1,8}$",
			onerror:"<?php echo L('maxmessagenum').L('between_1_to_8_num')?>"
		});
	$("#allowpostnum")
		.formValidator({
			tipid:"allowpostnumip",
			onshow:"<?php echo L('input').L('allowpostnum')?>",
			onfocus:"<?php echo L('allowpostnum').L('between_1_to_8_num')?>"
		})
		.regexValidator({
			regexp:"^\\d{1,8}$",
			onerror:"<?php echo L('allowpostnum').L('between_1_to_8_num')?>"
		});
});
//-->
</script>
...
<form name="myform" action="?app=member&controller=member_group&action=add" method="post" id="myform">
<fieldset>
		...
		<tr>
			...
			<td>
				<span class="ik lf" style="width:120px;">
					...
					<input type="text" name="info[price_d]" class="input-text" size="6" value="0.00">
				</span>
				<span class="ik lf" style="width:120px;">
					<?php echo L('member_group_monthprice')?>：
					<input type="text" name="info[price_m]" class="input-text" size="6" value="0.00">
				</span>
				<span class="ik lf" style="width:120px;">
					<?php echo L('member_group_yearprice')?>：
					<input type="text" name="info[price_y]" class="input-text" size="6" value="0.00">
				</span>
			</td>
		</tr>
		<tr>
