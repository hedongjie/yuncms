...
$(function(){
	...
	$("#username")
		...
		.ajaxValidator({
	    	type : "get",
			url : "api.php?controller=member&action=checkname",
			data :"",
			datatype : "html",
			async:'false',
	...
	$("#password")
		...
		.inputValidator({
			min:6,
			max:20,
			onerror:"<?php echo L('password').L('between_6_to_20')?>"
		});
	$("#pwdconfirm")
		.formValidator({
	...
	$("#email")
		...
		.ajaxValidator({
	    	type : "get",
			url : "api.php?controller=member&action=checkemail",
			data :"",
			datatype : "html",
			async:'false',
			success : function(data){
	...
	$("#mobile")
		.formValidator({
			empty:true,
			onshow:"<?php echo L('input').L('mobile')?>",
		...
		.defaultPassed();
	$("#nickname")
		.formValidator({empty:true,	onshow:"<?php echo L('input').L('nickname')?>",onfocus:"<?php echo L('nickname').L('between_2_to_20')?>"})
		.inputValidator({min:2,max:20,onerror:"<?php echo L('nickname').L('between_2_to_20')?>"})
		.regexValidator({regexp:"username",datatype:"enum",onerror:"<?php echo L('nickname').L('format_incorrect')?>"});
});
//-->
</script>
<div class="pad-10">
<div class="common-form">
<form name="myform" action="?app=member&controller=member&action=add" method="post" id="myform">
<fieldset>
	...
	<table width="100%" class="table_form">
		...
		<tr>
			...
			<td><input type="text" name="info[mobile]" id="mobile" value="<?php echo $memberinfo['mobile']?>" class="input-text"></input></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
			<?php echo Form::select($grouplist, '5', 'name="info[groupid]"', '');?>
			</td>
		</tr>
		<tr>
