...
function check_email($email, $userid = null) {
	if (! Validate::is_email ( $email )) return - 4; // 检查格式
	if (! check_denyemail ( $email )) return - 5; // 检查禁用
	if (! is_null ( $userid )) { // ID不为空
		$r = Loader::model ( 'member_model' )->getby_email ( array ('email' => $email ) ); // 检查会员表是否存在
		if ($r && $r ['userid'] != $userid) return - 6;
		$r = Loader::model ( 'member_verify_model' )->getby_email ( array ('email' => $email ) ); // 检查会员表是否存在
		if ($r && $r ['userid'] != $userid) return - 6;
	} else { // ID为空
		if (Loader::model ( 'member_model' )->getby_email ( array ('email' => $email ) )) return - 6;
		if (Loader::model ( 'member_verify_model' )->getby_email ( array ('email' => $email ) )) return - 6;
		if (ucenter_exists ()) {
