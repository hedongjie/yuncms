...
<form name="myform" id="myform" action="?app=special&controller=content&action=edit&specialid=<?php echo $_GET['specialid']?>&id=<?php echo $_GET['id']?>" method="post" enctype="multipart/form-data">
...
<div class="col-right">
	...
	<h6> <?php echo L('show_template')?></h6> <span id="show_template"><?php if ($data['style']) echo '<script type="text/javascript">$.getJSON(\'?app=admin&controller=category&action=public_tpl_file_list&style='.$data['style'].'&id='.$data['show_template'].'&application=special&templates=show&name=data\', function(data){$(\'#show_template\').html(data.show_template);});</script>'?></span>
          </div>
        </div>
    </div>
    <div class="col-auto">
...
<table width="100%" cellspacing="0" class="table_form">
	...
	<tr>
      ...
      <td><input type="text" style="width:350px;" name="info[title]" id="title" value="<?php echo htmlspecialchars($info['title'])?>" class="measure-input " onBlur="$.post('api.php?c=keyword&a=get&number=3&sid='+Math.random()*5, {data:$('#title').val()}, function(data){if(data && $('#keywords').val()=='') $('#keywords').val(data); })" />
		...
		<input type="button" class="button" id="check_title_alt" value="<?php echo L('check_exist')?>" onclick="$.get('?app=special&controller=content&action=public_check_title&sid='+Math.random()*5, {data:$('#title').val()}, function(data){if(data=='1') {$('#check_title_alt').val('<?php echo L('title_exist')?>');$('#check_title_alt').css('background-color','#FFCC66');} else if(data=='0') {$('#check_title_alt').val('<?php echo L('title_no_exist')?>');$('#check_title_alt').css('background-color','#F8FFE1')}})"/> <img src="static/image/icon/colour.png" width="15" height="16" onclick="colorpicker('title_colorpanel','set_title_color');" style="cursor:hand"/>
		<img src="statics/image/icon/bold.png" width="10" height="10" onclick="input_font_bold()" style="cursor:hand"/> <span id="title_colorpanel" style="position:absolute; z-index:200" class="colorpanel"></span>  </td>
    </tr>
    <tr>
