...
<input type="hidden" value="listinfo" name="action">
<input type="hidden" value="1" name="search">
<input type="hidden" value="<?php echo $_SESSION['hash']?>" name="hash">
...
<div class="table-list comment">
	...
	?>
     <tr id="tbody_<?php echo $v['id']?>">
		...
		<td width="230"><a href="?app=comment&controller=comment_admin&action=listinfo&search=1&searchtype=0&keyword=<?php echo urlencode($comment_info['title'])?>&hash=<?php echo $_SESSION['hash']?>&tableid=<?php echo $tableid?>"><?php echo $comment_info['title']?></td>
		<td align='center' width="72"><a href="?app=comment&controller=comment_admin&action=del&ids=<?php echo $v['id']?>&tableid=<?php echo $tableid?>&dosubmit=1" onclick="return check(<?php echo $v['id']?>, -1, '<?php echo $v['commentid']?>')"><?php echo L('delete');?></a> </td>
	</tr>
     <?php }
	}
	?>
...
function show_tbl(obj) {
	var pdoname = $(obj).val();
	location.href='?app=comment&controller=comment_admin&action=listinfo&tableid='+pdoname+'&hash=<?php echo $_SESSION['hash']?>';
}
