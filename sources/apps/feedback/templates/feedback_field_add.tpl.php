...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('fieldname_was_wrong');?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "app=feedback&controller=feedback_field&action=public_checkfield",
		datatype : "html",
		cached:false,
...
　<?php if (isset($formid) && !empty($formid)) {?><a href="?app=feedback&controller=feedback_field&action=init&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_field');?></em></a><?php } else {?><a href="?app=feedback&controller=feedback_field&action=init&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('field_manage')?></em></a><?php }?></div>
  <div class="bk10"></div>
</div>
<form name="myform" id="myform" action="?app=feedback&controller=feedback_field&action=add" method="post">
...
<script type="text/javascript">
	...
	$('#css').css('display','none');
		...
		$.getJSON("?app=feedback&controller=feedback_field&action=public_field_setting&fieldtype="+fieldtype, function(data){
