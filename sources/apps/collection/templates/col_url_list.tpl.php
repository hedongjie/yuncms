...
<?php if ($total_page > $page) {
	echo  "<script type='text/javascript'>location.href='?app=collection&controller=node&action=col_url_list&page=".($page+1)."&nodeid=$nodeid'</script>";
} else {?>
	<script type="text/javascript">
	window.top.art.dialog({id:'test'}).close();
	window.top.art.dialog({id:'test',content:'<h2><?php echo L('collection_success')?></h2><span style="fotn-size:16px;"><?php echo L('following_operation')?></span><br /><ul style="fotn-size:14px;"><li><a href="?app=collection&controller=node&action=col_content&nodeid=<?php echo $nodeid?>" target="right"  onclick="window.top.art.dialog({id:\'test\'}).close()"><?php echo L('following_operation_1')?></a></li><li><a href="?app=collection&controller=node&action=manage&menuid=212" target="right" onclick="window.top.art.dialog({id:\'test\'}).close()"><?php echo L('following_operation_2')?></a></li></ul>',width:'400',height:'200'});
	</script>
<?php }?>
</div>
</div>
