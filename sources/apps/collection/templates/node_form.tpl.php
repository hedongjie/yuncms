...
$(document).ready(function() {
	...
	$("#name").formValidator({onshow:"<?php echo L('input').L('nodename')?>",onfocus:"<?php echo L('input').L('nodename')?>"}).inputValidator({min:1,onerror:"<?php echo L('input').L('nodename')?>"}).ajaxValidator({type : "get",url : "",data :"app=collection&controller=node&action=public_name<?php if(ACTION=='edit')echo "&nodeid=$data[nodeid]"?>",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('nodename').L('exists')?>",onwait : "<?php echo L('connecting')?>"})<?php if(ACTION=='edit')echo ".defaultPassed()"?>;
...
<form name="myform" action="?app=collection&controller=node&action=<?php echo ACTION?>&nodeid=<?php if(isset($nodeid)) echo $nodeid?>" method="post" id="myform">
...
function show_url() {
	...
	window.top.art.dialog.open('?app=collection&controller=node&action=public_url&sourcetype='+type+'&urlpage='+$('#urlpage_'+type).val()+'&pagesize_start='+$("input[name='data[pagesize_start]']").val()+'&pagesize_end='+$("input[name='data[pagesize_end]']").val()+'&par_num='+$("input[name='data[par_num]']").val() ,{
