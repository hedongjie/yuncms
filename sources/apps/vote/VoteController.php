...
class VoteController extends admin {
	...
	public function setting() {
		...
		$data = $m_db->select ( array ('application' => 'vote' ) );
		$now_seting = string2array ( $data [0] ['setting'] );
		if (isset ( $_POST ['dosubmit'] )) {
			$setting = $_POST ['setting'];
			W ( 'common/vote', $setting );
			// 更新模型数据库,重设setting 数据.
			$set = array2string ( $setting );
			$m_db->update ( array ('setting' => $set ), array ('application' => ROUTE_APP ) );
	...
	public function public_get_votelist() {
		$infos = $this->db->listinfo ( '', 'subjectid DESC', 1, '10' );
