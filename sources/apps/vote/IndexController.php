...
class IndexController {
	function __construct() {
		...
		$this->username = cookie ( '_username' );
		$this->userid = cookie ( '_userid' ) ? cookie ( '_userid' ) : 0;
	...
	public function show() {
		...
		// 获取投票结果信息
		$infos = $this->vote_data->where(array ('subjectid' => $subjectid ))->field('data')->select (  );
		// 循环每个会员的投票记录
	...
	public function check($subjectid) {
		...
		// 是否有投票记录
		$user_info = $this->vote_data->where(array ('subjectid' => $subjectid,'ip' => $this->ip,'username' => $this->username ))->order('time DESC')->limit(1)->select ();
		if (! $user_info) {
			return 1;
		} else {
