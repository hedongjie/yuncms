...
<!--
	...
	$("#fromdate").formValidator({onshow:"<?php echo L('select').L('fromdate')?>",onfocus:"<?php echo L('select').L('fromdate')?>",oncorrect:"<?php
...
<div class="pad_10">
	<form action="<?php echo U('vote/vote/add');?>" method="post"
		...
		<table cellpadding="2" cellspacing="1" class="table_form" width="100%">
			...
			<tr id="SizeFormat" style="display: none;">
				<th></th>
				<td><label><?php echo L('minval')?></label>&nbsp;&nbsp;<input
			...
			<tr>
				...
				<td><input type="button" id="addItem"
					value="<?php echo L('add_option')?>" class="button"
					onclick="add_option()">

					<div id="option_list_1">
						<div>
							<br> <input type="text" name="option[]" id="option1" size="40"
								require="true" id="opt1" />
						</div>

						<div>
							<br> <input type="text" name="option[]" id="option2" size="40"
								id="opt2" />
						</div>
			...
			<tr>
				...
				<td><?php echo Form::date('subject[fromdate]', date('Y-m-d',TIME), '')?></td>
			</tr>
			<tr>
				<th><?php echo L('todate')?> :</th>
				<td><?php echo Form::date('subject[todate]', date('Y-m-d',TIME+2592000), '')?></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input name="subject[allowguest]" type="radio" value="1"
					<?php if($allowguest == 1) {?> checked <?php }?>>&nbsp;<?php echo L('yes')?>&nbsp;&nbsp;<input
					name="subject[allowguest]" type="radio" value="0"
					<?php if($allowguest == 0) {?> checked <?php }?>>&nbsp;<?php echo L('no')?></td>
			</tr>
			<tr>
				<th><?php echo L('credit')?>：</th>
				<td><input name="subject[credit]" type="text"
					value="<?php echo $credit;?>" size='5'></td>
			</tr>

			<tr>
				<th><?php echo L('interval')?>： </th>
				<td><input type="text" name="subject[interval]"
					value="<?php echo $interval;?>" size='5' /> <?php echo L('more_ip')?>，<font
		...
		<?php echo Form::select($template_list, C('template','name'), 'name="vote_subject[style]" id="style" onchange="load_file_list(this.value)"', L('please_select'))?>
			...
			<tr>
				...
				<td id="show_template">
		<?php echo Form::select_template(C('template','name'), 'vote', $vote_tp_template, 'name="vote_subject[vote_tp_template]"', 'vote_tp');?>
		</td>
			</tr>
			<tr>
				<th><?php echo L('enabled')?>：</th>
				<td><input name="subject[enabled]" type="radio" value="1"
					<?php if($enabled == 1) {?> checked <?php }?>>&nbsp;<?php echo L('yes')?>&nbsp;&nbsp;<input
					name="subject[enabled]" type="radio" value="0"
					<?php if($enabled == 0) {?> checked <?php }?>>&nbsp;<?php echo L('no')?></td>
			</tr>

			<tr>
				<th></th>
				<td><input type="hidden" name="from_api"
					value="<?php echo $_GET['from_api'];?>"> <input type="submit"
					name="dosubmit" id="dosubmit" class="dialog"
...
function load_file_list(id) {
	$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&style='+id+'&application=vote&templates=vote_tp&name=vote_subject', function(data){$('#show_template').html(data.vote_tp_template);});
}
</script>