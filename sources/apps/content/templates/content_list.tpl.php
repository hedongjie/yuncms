...
<!--
	...
	parent.document.getElementById('center_frame').src = '?app=content&controller=content&action=public_categorys&type=add&menuid=<?php echo $_GET['menuid'];?>';
...
<a class="add fb" href="javascript:;" onclick=javascript:openwinx('?app=content&controller=content&action=add&menuid=&catid=<?php echo $catid;?>','')><em><?php echo L('add_content');?></em></a>
<a href="?app=content&controller=content&action=init&catid=<?php echo $catid;?>" <?php if($steps==0 && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('check_passed');?></em></a><span>|</span>
...
<span>|</span><a href="?app=content&controller=create_html&action=category&pagesize=30&dosubmit=1&modelid=0&catids[0]=<?php echo $catid;?>&referer=<?php echo urlencode($_SERVER['QUERY_STRING']);?>"><em><?php echo L('update_htmls',array('catname'=>$category['catname']));?></em></a>
<?php }?>
</div>
...
<div class="table-list">
    <table width="100%">
        <thead>
            ...
            <th width="138"><?php echo L('updatetime');?></th>
...
<tbody>
	...
	if(is_array($datas)) {
	    ...
	    $release_siteurl = substr(SITE_URL,0,$path_len);
		$this->hits_db = Loader::model('hits_model');
		foreach ($datas as $r) {
			$hits_r = $this->hits_db->where(array('hitsid'=>'c-'.$modelid.'-'.$r['id']))->find();
	?>
        <tr>
		...
		if($status==99) {
			...
			echo '<a href="javascript:;" onclick=\'window.open("?app=content&controller=content&action=public_preview&steps='.$steps.'&catid='.$catid.'&id='.$r['id'].'","manage")\'>';
		...
		<?php
		if($r['sysadd']==0) {
			echo "<a href='?app=member&controller=member&action=memberinfo&username=".urlencode($r['username'])."' >".$r['username']."</a>";
		...
		<td align='center'><a href="javascript:;" onclick="javascript:openwinx('?app=content&controller=content&action=edit&catid=<?php echo $catid;?>&id=<?php echo $r['id']?>','')"><?php echo L('edit');?></a> | <a href="javascript:view_comment('<?php echo id_encode('content_'.$catid,$r['id']);?>','<?php echo safe_replace($r['title']);?>')"><?php echo L('comment');?></a></td>
	</tr>
     <?php }
	}
	?>
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
		<input type="hidden" value="<?php echo $hash;?>" name="hash">
    	<input type="button" class="button" value="<?php echo L('listorder');?>" onclick="myform.action='?app=content&controller=content&action=listorder&dosubmit=1&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';myform.submit();"/>
		<?php if($category['content_ishtml']) {?>
		<input type="button" class="button" value="<?php echo L('createhtml');?>" onclick="myform.action='?app=content&controller=create_html&action=batch_show&dosubmit=1&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';myform.submit();"/>
		<?php }
		if($status!=99) {?>
		<input type="button" class="button" value="<?php echo L('passed_checked');?>" onclick="myform.action='?app=content&controller=content&action=pass&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';myform.submit();"/>
		<?php }?>
		<input type="button" class="button" value="<?php echo L('delete');?>" onclick="myform.action='?app=content&controller=content&action=delete&dosubmit=1&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';return confirm_delete()"/>
		...
		<?php }}?>
		<input type="button" class="button" value="<?php echo L('remove');?>" onclick="myform.action='?app=content&controller=content&action=remove&catid=<?php echo $catid;?>';myform.submit();"/>
	</div>
...
function push() {
	...
	window.top.art.dialog.open('?app=content&controller=push&act=position_list&catid=<?php echo $catid?>&modelid=<?php echo $modelid?>&id='+id,{
...
function view_comment(id, name) {
	window.top.art.dialog.open('index.php?app=comment&controller=comment_admin&action=lists&show_center_id=1&commentid='+id,{
...
function reject_check(type) {
	if(type==1) {
		...
		document.getElementById('myform').action='?app=content&controller=content&action=pass&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>&reject=1';
