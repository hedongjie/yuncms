...
function(){
	$('#cat_search').keyup(
		function(){
			...
			if (value.length > 0){
				$.getJSON('?app=admin&controller=category&action=public_ajax_search', {catname: value}, function(data){
					if (data != null) {
						var str = '';
						$.each(data, function(i,n){
							if(n.type=='0') {
								str += '<li><a href="?app=content&controller=content&action=init&menuid=176&catid='+n.catid+'">'+n.catname+'</a></li>';
							} else {
								str += '<li><a href="?app=content&controller=content&action=add&menuid=176&catid='+n.catid+'">'+n.catname+'</a></li>';
							}
						});
						$('#search_div').html(str);
						$('#search_div').show();
