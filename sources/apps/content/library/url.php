...
class url {
	...
	public function __construct() {
		$this->urlrules = S ( 'common/urlrule' );
	...
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '', $data = '', $action = 'edit', $upgrade = 0) {
		...
		if ($upgrade || (isset ( $_POST ['upgrade'] ) && defined ( 'IN_ADMIN' ) && $_POST ['upgrade'])) {
			...
			if (strpos ( $category ['url'], '://' ) !== false && strpos ( $category ['url'], '?' ) === false) {
				...
				$db = Loader::model ( 'category_model' );
				$r = $db->field ( 'catid' )->where ( array ('url' => $url ) )->find ();
			...
			$urls = str_replace ( array ('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$page}','{$prefix}' ), array ($categorydir,$catdir,$year,$month,$day,$catid,$id,$page,$prefix ), $urlrule );
			$create_to_html_root = $category ['create_to_html_root'];

			if ($create_to_html_root || $category ['sethtml'] == 1) {
				$html_root = '';
			} else {
				$html_root = $this->html_root;
			}
			if ($content_ishtml == 1 && $url) {
	...
	public function category_url($catid, $page = 1) {
		...
		if (in_array ( basename ( $url ), array ('index.html','index.shtml','index.htm' ) )) {
			$url = dirname ( $url ) . '/';
		}
