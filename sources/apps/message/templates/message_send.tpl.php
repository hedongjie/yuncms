...
<!--
	...
	$("#subject")
		.formValidator({
			...
			onfocus:"<?php echo L('input','',admin).L('subject')?>"
		})
		.inputValidator({
			min:1,
			onerror:"<?php echo L('input','',admin).L('subject')?>"
		});
	$("#content")
		.formValidator({
			onshow:"<?php echo L('input','',admin).L('content')?>",
			onfocus:"<?php echo L('input','',admin).L('content')?>"
		})
		.inputValidator({
			min:1,
			onerror:"<?php echo L('input','',admin).L('content')?>"
		});
	})
//-->
</script>
...
<div class="pad_10">
<form action="?app=message&controller=message&action=message_send" method="post" name="myform" id="myform">
