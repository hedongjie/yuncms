...
<style>
.arrowhead,.arrowhead-b {
	background: url(<?php echo IMG_PATH?>icon/arrowhead.png) no-repeat;
	height: 15px;
	width: 16px;
}
...
a.close {
	background: url(<?php echo IMG_PATH?>cross.png) no-repeat left -46px;
	display: block;
	width: 16px;
	height: 16px;
...
<script type="text/javascript"
	src="<?php echo JS_PATH?>swfupload/swf2ckeditor.js"></script>
<?php endif;?>

<div class="pad-10">
	<form
		action="?app=block&controller=admin&action=block_update&id=<?php echo $id?>"
...
editAreaLoader.init({
  ...
  <tr>
					<td class="y-bg"><textarea cols="80" id="data" name="data"
							rows="20"><?php echo htmlspecialchars($data['data'])?></textarea></td>
				</tr>
<?php else:?>
<?php if(is_array($data['data'])) foreach($data['data'] as $key=>$val):?>
  <tr>
					<td>
						<div class="contents" id="content_<?php echo $key?>">
							<a href="<?php echo $val['url']?>" target="blank"><b><?php echo $val['title']?></b></a><br />
							<div style="color: #ccc;"><?php if($val['thumb']):?><img
									src="<?php echo $val['thumb']?>" class="thumb" /><?php endif;?><?php echo $val['desc']?></div>
						</div>
						<div class="forms" id="form_<?php echo $key?>">
    <?php echo L('title')?>：<input type="text"
								id="title_<?php echo $key?>" name="title[]" class="input-text"
								value="<?php echo $val['title']?>"> <?php echo L('link')?>：<input
								type="text" id="url_<?php echo $key?>" name="url[]"
								class="input-text" value="<?php echo $val['url']?>"> <?php echo L('thumb')?>：<input
								type="hidden" name="thumb[]" id="thumb_<?php echo $key?>"
								value="<?php echo $val['thumb']?>"> <a href="javascript:void(0)"
								onclick="flashupload('thumb_images', '<?php echo L('attachment_upload')?>','thumb_<?php echo $key?>',submit_images,'1,jpg|jpeg|gif|bmp|png,1','block', '', '<?php echo upload_key('1,jpg|jpeg|gif|bmp|png,1')?>')"><?php echo L('pic_upload')?></a>
							<a href="javascript:void(0)"
								onclick="$('#thumb_<?php echo $key?>').val('')"><?php echo L('delete_image')?></a><br /><?php echo L('desc')?>：<textarea
								id="desc_<?php echo $key?>" name="desc[]" rows="10" cols="80"><?php echo str_replace(array(chr(13), chr(43)), array('<br />', '&nbsp;'), $val['desc'])?></textarea>
							<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input
								type="button" value="<?php echo L('submit')?>" class="button"
								onclick="form_submit(<?php echo $key?>)" />
						</div>
					</td>
					<td width="80"><a href="javascript:void(0);" class="arrowhead"
						onclick="moveUp(this);" title="<?php echo L('up')?>"></a><a
						href="javascript:void(0);" onclick="moveDown(this);"
						class="arrowhead-b" title="<?php echo L('down')?>"></a><a
						href="javascript:void(0)" onclick="edit_form(<?php echo $key?>)"><img
							src="<?php echo IMG_PATH?>icon/m_2.png"
							alt="<?php echo L('edit')?>" /></a><a href="javascript:void(0)"
						onclick="$(this).parent().parent().remove();" class="close"
						title="<?php echo L('delete')?>"></a></td>
				</tr>
<?php endforeach;endif;?>
</table>
			<div class="bk15"></div>
			<input type="button" value="<?php echo L('preview')?>" class="button"
				onclick="block_view(<?php echo $id?>)" /> <input type="button"
				value="<?php echo L('history')?>" class="button"
				onclick="show_history()" /> <input type="button"
				value="<?php echo L('search_content')?>" class="button"
				onclick="search_content()">
<?php if ($data['type'] == 2) :?><?php echo L('datatable')?>: <input
				type="text" id="linenum" size="2" value="1" /><input type="button"
				value="<?php echo L('submit')?>" class="button" onclick="add_line()" />
		</fieldset>
		<div class="bk15"></div>
		<fieldset>
			...
			<table width="100%" class="table_form">
				<tr>
					<Td><textarea name="template" id="template"
							style="width: 100%; height: 120px;"><?php if ($data['template']) :echo htmlspecialchars($data['template']); else:?>{$name}
<ul>
{loop $data $i $r}
<li style="clear: both">
...
{/loop}
					...
					<td width="120"><span style="height: 25px"><input type="button"
							class="button" value="{loop }" title="<?php echo L('insert')?>"
							style="width: 50px"
							onClick="javascript:insertText('{loop $data $n $r}')" /></span> <span
						style="height: 25px"><input type="button" class="button"
							value="{/loop}" title="<?php echo L('insert')?>"
							style="width: 50px" onClick="javascript:insertText('{/loop}')" /></span><br />
						<span style="height: 25px"><input type="button" class="button"
							value="<?php echo L('name')?>" title="<?php echo L('insert')?>"
							style="width: 50px" onClick="javascript:insertText('{$name}')" /></span>
						<span style="height: 25px"><input type="button" class="button"
							value="<?php echo L('title')?>" title="<?php echo L('insert')?>"
							style="width: 50px"
							onClick="javascript:insertText('{$r[title]}')" /></span><br /> <span
						style="height: 25px"><input type="button" class="button"
							value="URL" title="<?php echo L('insert')?>" style="width: 50px"
							onClick="javascript:insertText('{$r[url]}')" /></span> <span
						style="height: 25px"><input type="button" class="button"
							value="<?php echo L('thumb')?>" title="<?php echo L('insert')?>"
							style="width: 50px"
							onClick="javascript:insertText('{$r[thumb]}')" /></span><br /> <span
						style="height: 25px"><input type="button" class="button"
							value="<?php echo L('desc')?>" title="<?php echo L('insert')?>"
							style="width: 50px" onClick="javascript:insertText('{$r[desc]}')" /></span>
					</td>
				</tr>
			</table>
		</fieldset>
 ...
 <div class="bk15"></div>
        ...
        <tr>
							...
							<td><a
								href="?app=block&controller=admin&action=history_restore&id=<?php echo $val['id']?>"><?php echo L('restore')?></a>
								<a
								href="<?php echo art_confirm(L('are_you_sure_you_want_to_delete'), '?app=block&controller=admin&action=history_del&id='.$val['id']);?>"><?php echo L('delete')?></a></td>
						</tr>
        <?php endforeach;?>
</tbody>
				</table>
	...
	<?php if($pages):?><div id="pages"><?php echo $pages;?></div><?php endif;?>
		...
		<input type="submit" class="dialog" id="dosubmit" name="dosubmit"
			value="" />
...
function block_view(id) {
	...
	$('#myform').attr('action', '?app=block&controller=admin&action=public_view&id='+id);
...
function search_content() {
	art.dialog.open("?app=block&controller=admin&action=public_search_content",{
		title:'<?php echo L('search_content')?>',
		id:'search_content',
		width:'600px',
		height:'400px'
	});
