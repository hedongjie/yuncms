...
<!--
	$(function(){
		...
		$("#name")
			...
			.ajaxValidator({
				type : "get",
				url : "?app=block&controller=admin&action=public_name&id=<?php if(isset($id) && !empty($id))echo $id;?>",
				data :"",
				datatype : "html",
				async:'false',
...
<div class="pad-10">
	<form
		action="?app=block&controller=admin&action=<?php echo ACTION?>&pos=<?php echo $_GET['pos']?>&id=<?php if(isset($id) && !empty($id))echo $id;?>"
			...
			<fieldset>
				...
				<table width="100%" class="table_form">
					<tr>
						...
						<td class="y-bg"><input type="text" name="name" id="name"
							size="30"
							value="<?php echo isset($data['name']) ?  $data['name'] : '';?>" /></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="y-bg"><?php echo Form::radio(array('1'=>L('code'), '2'=>L('table_style')), (isset($data['type']) ? $data['type'] : 1), 'name="type"'.(ACTION=='edit' ? ' disabled = "disabled"' : ''))?></td>
					</tr>
				</table>
			</fieldset>
			<div class="bk15"></div>
