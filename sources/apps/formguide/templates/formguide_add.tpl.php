...
?>
<div class="pad-10">
<form method="post" action="<?php echo U('formguide/formguide/add');?>" name="myform" id="myform">
...
<tbody>
	...
	<tr id="time_start" style="display:none;">
        ...
        <td><?php echo Form::date('setting[starttime]', date('Y-m-d', TIME))?></td>
	</tr>
	...
	<tr>
		...
		<td>
		<?php echo Form::select($template_list, C('template','name'), 'name="info[default_style]" id="style" onchange="load_file_list(this.value)"', L('please_select'))?>
		</td>
	</tr>
	<tr>
		<th><strong><?php echo L('template_selection')?>：</strong></th>
		<td id="show_template"><script type="text/javascript">$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&style=<?php echo C('template','name')?>&application=formguide&templates=show&name=info', function(data){$('#show_template').html(data.show_template);});</script></td>
	</tr>
	<tr>
		<th><strong>js调用使用的模板：</strong></th>
		<td id="show_js_template"><script type="text/javascript">$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&style=<?php echo C('template','name')?>&application=formguide&templates=show_js&name=info', function(data){$('#show_js_template').html(data.show_js_template);});</script></td>
	</tr>
	</tbody>
</table>
...
function load_file_list(id) {
	if (id=='') return false;
	$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&style='+id+'&application=formguide&templates=show|show_js&name=info', function(data){$('#show_template').html(data.show_template);$('#show_js_template').html(data.show_js_template);});
}
...
$(document).ready(function(){
	...
	$('#tablename').formValidator({onshow:"<?php echo L('please_input_tallename')?>", onfocus:"<?php echo L('standard')?>", oncorrect:"<?php echo L('right')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('tablename_was_wrong');?>"}).inputValidator({min:1,onerror:"<?php echo L('tablename_no_empty')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "app=formguide&controller=formguide&action=public_checktable",
		datatype : "html",
		cached:false,
