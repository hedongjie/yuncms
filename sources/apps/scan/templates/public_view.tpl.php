...
<div class="pad-10" style="padding-bottom: 0px;">
  <table width="100%" height="400" class="table_form">
    <?php if (isset($func)) :?>
    <tr>
      <td><?php echo L('characteristic_function')?>：
        <?php
	if ($func) {
		foreach ( $func as $val ) {
			if ($val) {
    ...
    <?php if (isset($code)) :?>
    <tr>
      <td><?php echo L('characteristic_key')?>：
        <?php
	if ($code) {
		foreach ( $code as $val ) {
			if ($val) {
    ...
    <?php endif;?>
    <tr>
      <td><textarea name="code" id="code"
					style="width: 650px; height: 380px;"><?php echo htmlspecialchars($html)?></textarea></td>
    </tr>
  </table>
</div>
