 ...
 * @version $Id: spend.php 61 2012-11-05 12:48:43Z xutongle $
 *
 * 直接使用Loader::lib('pay:spend',false);
 * 进行加载。
...
class spend {
    ...
    protected  static function connect() {
        self::$db = Loader::model("pay_spend_model");
    }
    ...
    public static function spend_time($userid, $time, $logo) {
        if (empty(self::$db)) self::connect();
        return self::$db->get_one("`userid` = '$userid' AND `creat_at` BETWEEN '$time' AND '".TIME."' AND `logo` = '$logo'");
    }
    ...
    private static function _add($data) {
        ...
        //检察消费类型
        if (!in_array($data['type'], array(1,2))) return false;
        ...
        if (defined('IN_ADMIN') && (empty($data['op_userid']) || empty($data['op_username']))) {
            $data['op_username'] = cookie_get('admin_username');
            $data['op_userid'] = cookie_get('userid');
        }
