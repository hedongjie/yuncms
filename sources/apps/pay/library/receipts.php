 ...
 * @version $Id: receipts.php 61 2012-11-05 12:48:43Z xutongle $
 */
class receipts {
    ...
    private static function _add($data) {
        ...
        //检查入账类型
        if (!in_array($data['pay_type'], array('offline','recharge','selfincome'))) return false;
        //检查入账状态
        if (!in_array($data['status'], array('succ','error','failed'))) return false;

        //检查消费描述
        if (empty($data['payment'])) return false;

        //检查消费金额
        if (empty($data['money'])) return false;
