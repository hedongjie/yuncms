 ...
 * @version $Id: pay_method.php 521 2012-12-02 17:10:08Z xutongle $
 */
class pay_method {
	...
	public function get_payment($code = '') {
		$modules = $this->read_payment ( $this->modules_path );
		$payment_info = array ();
		foreach ( $modules as $payment ) {
			...
			$payment_info [$payment ['code']] = array ("pay_id" => 0,"pay_code" => $payment ['code'],"pay_name" => $payment ['name'],"pay_desc" => $payment ['desc'],"pay_fee" => '0',"config" => $config,"is_cod" => $payment ['is_cod'],"is_online" => $payment ['is_online'],"enabled" => '0',"sort_order" => "","author" => $payment ['author'],"website" => $payment ['website'],"version" => $payment ['version'] );
		}
		if (empty ( $code )) {
			return $payment_info;
		} else {
