...
<!--
	$(function(){
		...
		$("#catdir")
			...
			.ajaxValidator({
				type : "get",
				url : "?app=admin&controller=category&action=public_check_catdir&old_dir=<?php echo $catdir;?>",
				data :"",
				datatype : "html",
				async:'false',
...
<form name="myform" id="myform"
	action="?app=admin&controller=category&action=edit" method="post">
	<div class="pad-10">
		<div class="col-tab">
			...
			<div id="div_setting_1" class="contentList pad-10">
				...
				<table width="100%" class="table_form ">
					<tr>
						...
						<td>
		 <?php
			$category_items = S ( 'common/category_items_' . $modelid );
		...
		<?php echo Form::select_category('category_content',$parentid,'name="info[parentid]"',L('please_select_parent_category'),0,-1);?>
					...
					<tr>
						...
						<td><input type="text" name="info[catname]" id="catname"
							class="input-text" value="<?php echo $catname;?>"></td>
					</tr>
					<tr>
						<th><?php echo L('catdir')?>：</th>
						<td><input type="text" name="info[catdir]" id="catdir"
							class="input-text" value="<?php echo $catdir;?>"></td>
					</tr>
					<tr>
	  ...
	  <?php
			...
			?>
					...
					<tr>
						...
						<td><input type='radio' name='info[ismenu]' value='1'
							<?php if($ismenu) echo 'checked';?>> <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='info[ismenu]' value='0'
							<?php if(!$ismenu) echo 'checked';?>> <?php echo L('no');?></td>
					</tr>
	  ...
	  （<?php echo L('create_to_rootdir_tips');?>）</td>
					...
					<tr id="tr_domain" style="display:<?php if(!$setting['ishtml']) echo 'none';?>">
						...
						<td><input type="text" name="info[url]" id="url"
							...
							value="<?php if(preg_match('/^http:\/\/([a-z0-9\-\.]+)\/$/i',$url)) echo $url;?>"></td>
					</tr>
				</table>
			</div>
		...
		<?php echo Form::select($template_list, $setting['template_list'], 'name="setting[template_list]" id="template_list" onchange="load_file_list(this.value)"', L('please_select'))?>
					...
					<tr>
        				<th width="200"><?php echo L('template_child');?></th>
				...
				foreach ( $roles as $roleid => $rolrname ) {
		  		...
		  		<tr>
										...
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											<?php echo $this->check_category_priv('init',$roleid);?>
											value="init,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											<?php echo $this->check_category_priv('add',$roleid);?>
											value="add,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											<?php echo $this->check_category_priv('edit',$roleid);?>
											value="edit,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											<?php echo $this->check_category_priv('delete',$roleid);?>
											value="delete,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											<?php echo $this->check_category_priv('listorder',$roleid);?>
											value="listorder,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											<?php echo $this->check_category_priv('push',$roleid);?>
											value="push,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											<?php echo $this->check_category_priv('move',$roleid);?>
				...
				?>
		  		<tr>
										...
										<td align="center"><input type="checkbox"
											name="priv_groupid[]"
											<?php echo $this->check_category_priv('visit',$_value['groupid'],0);?>
											value="visit,<?php echo $_value['groupid'];?>"></td>
										<td align="center"><input type="checkbox"
											name="priv_groupid[]"
											<?php echo $this->check_category_priv('add',$_value['groupid'],0);?>
				...
				<table width="100%" class="table_form">
					<tr>
						...
						<td><input name='setting[presentpoint]' type='text'
							...
							maxlength='5' style='text-align: center'> <?php echo L('contribute_add_point_tips');?></td>
					</tr>
					<tr>
						...
						<td><input name='setting[defaultchargepoint]' type='text'
							...
							name="setting[paytype]"><option value="0"
									<?php if(!$setting['paytype']) echo 'selected';?>><?php echo L('readpoint');?></option>
								<option value="1"
									<?php if($setting['paytype']) echo 'selected';?>><?php echo L('money');?></option></select> <?php echo L('readpoint_tips');?></td>
					</tr>
					<tr>
...
<!--
	...
	function load_file_list(id) {
		if(id=='') return false;
		$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&style='+id+'&catid=<?php echo $catid?>', function(data){$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
	}
