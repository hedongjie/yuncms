...
<?php echo L('category_batch_tips');?></a>
	...
	<?php
	 ...
	 <?php
		foreach ( $batch_array as $catid => $cat ) {
			?>
		<td><b><?php echo L('html_show')?>：</b><br> <input
							boxid="content_ishtml" catid="<?php echo $catid;?>" type='radio'
							name='setting[<?php echo $catid;?>][content_ishtml]' value='1'
							<?php if($cat['setting']['content_ishtml']) echo 'checked';?>
							onClick="change_radio(event,'content_ishtml',1,'show');urlrule('show',1,<?php echo $catid;?>)"> <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;
	  <input boxid="content_ishtml" catid="<?php echo $catid;?>"
							type='radio' name='setting[<?php echo $catid;?>][content_ishtml]'
							value='0'
							<?php if(!$cat['setting']['content_ishtml']) echo 'checked';?>
							onClick="change_radio(event,'content_ishtml',0,'show');urlrule('show',0,<?php echo $catid;?>)"> <?php echo L('no');?>
	  </td>
	<?php
		}
		?>
	...
	<?php
	 ...
	 <?php
		foreach ( $batch_array as $catid => $cat ) {
			?>
		<td><b><?php echo L('create_to_rootdir')?>：</b><br> <input
							boxid="create_to_html_root"
							onclick="change_radio(event,'create_to_html_root',1)"
							type='radio'
							name='setting[<?php echo $catid;?>][create_to_html_root]'
							value='1'
							<?php if($cat['setting']['create_to_html_root']) echo 'checked';?>> <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;
	  <input boxid="create_to_html_root"
							onclick="change_radio(event,'create_to_html_root',0)"
							type='radio'
							name='setting[<?php echo $catid;?>][create_to_html_root]'
							value='0'
							<?php if(!$cat['setting']['create_to_html_root']) echo 'checked';?>> <?php echo L('no');?>
	  </td>
	<?php
		}
		?>
	...
	<?php
	 ...
	 <?php
		foreach ( $batch_array as $catid => $cat ) {
			?>
		<td><b><?php echo L('contribute_add_point')?>：</b><br> <input
							name='setting[<?php echo $catid;?>][presentpoint]' type='text'
							value='<?php echo $cat['setting']['presentpoint'];?>'
							style='width: 100px' maxlength='60'>
		 <?php echo L('point');?> </td>
	<?php
		}
		?>
	 ...
	 <?php
		foreach ( $batch_array as $catid => $cat ) {
			?>
		<td><b><?php echo L('default_readpoint')?>：</b><br> <input
							name='setting[<?php echo $catid;?>][defaultchargepoint]'
							type='text'
							value='<?php echo $cat['setting']['defaultchargepoint'];?>'
							style='width: 100px' maxlength='60'> <select
							name="setting[<?php echo $catid;?>][paytype]"><option value="0"
									<?php if(!$cat['setting']['paytype']) echo 'selected';?>><?php echo L('readpoint');?></option>
								<option value="1"
									<?php if($cat['setting']['paytype']) echo 'selected';?>><?php echo L('money');?></option></select>
						</td>
	<?php
		}
		?>
	 ...
	 <?php
		foreach ( $batch_array as $catid => $cat ) {
			?>
		<td><b><?php echo L('repeatchargedays')?>：</b><br> <input
							name='setting[<?php echo $catid;?>][repeatchargedays]'
							type='text'
							value='<?php echo $cat['setting']['repeatchargedays'];?>'
							style='width: 100px' maxlength='60'><?php echo L('repeat_tips');?>
		  </td>
	<?php
		}
		?>
			...
			<div class="btn">
				<input type="hidden" name="type" value="<?php echo $type;?>" /> <input
					type="submit" class="button" name="dosubmit"
...
function load_file_list(id,catid) {
	if(id=='') return false;
	$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&batch_str=1&style='+id+'&catid='+catid, function(data){
