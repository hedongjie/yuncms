...
$(function(){
	...
	$("#title")
		.formValidator({
			onshow:"<?php echo L('input').L('posid_title')?>",
			onfocus:"<?php echo L('posid_title').L('not_empty')?>"
		})
		.inputValidator({
			min:1,
			max:999,
			onerror:"<?php echo L('posid_title').L('not_empty')?>"
		});
	$("#url")
		.formValidator({
			onshow:"<?php echo L('input').L('posid_url')?>",
			onfocus:"<?php echo L('posid_url').L('not_empty')?>"
		})
		.inputValidator({
			min:1,
			max:999,
			onerror:"<?php echo L('posid_url').L('not_empty')?>"
		});
})
//-->
</script>
<div class="pad_10">
	<div class="common-form">
		<form name="myform"
			action="?app=admin&controller=position&action=public_item_manage"
			...
			<table width="100%" class="table_form">
				<tr>
					...
					<td><input type="text" name="info[title]" class="input-text"
						value="<?php echo $title?>" id="title" size="40"></input></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><textarea name="info[description]" rows="2" cols="20"
							...
							style="height: 100px; width: 300px;"><?php echo $description?></textarea>
					</td>
				</tr>
				<tr>
					<td><?php echo L('posid_syn')?></td>
					<td><input name="synedit" value="0" type="radio"
						<?php echo $synedit==0 ? 'checked="checked"' : ''?>> <?php echo L('enable')?><input
						name="synedit" value="1" type="radio"
						<?php echo $synedit==1 ? 'checked="checked"' : ''?>> <?php echo L('close')?>
</td>
				</tr>
