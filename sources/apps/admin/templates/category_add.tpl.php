...
<!--
	$(function(){
		...
		$("#modelid")
			...
			.inputValidator({
				min:1,
				onerror:"<?php echo L('select_model');?>"
			});
		$("#catname")
			.formValidator({
		...
		$("#catdir")
			...
			.inputValidator({
				min:1,
				onerror:"<?php echo L('input_dirname');?>"
			})
			.ajaxValidator({type : "get",
				url : "?app=admin&controller=category&action=public_check_catdir",
				data :"",
				datatype : "html",
				async:'false',
		...
		$("#url")
			.formValidator({
				onshow:" ",onfocus:"<?php echo L('domain_name_format');?>",
				tipcss:{width:'300px'},
				empty:true
			})
			.inputValidator({
				onerror:"<?php echo L('domain_name_format');?>"
			})
			.regexValidator({
				regexp:"http:\/\/(.+)\/$",
		...
		$("#template_list")
			...
			.inputValidator({
				min:1,
				onerror:"<?php echo L('template_setting');?>"
			});
	})
//-->
</script>
...
<form name="myform" id="myform"
	action="<?php echo U('admin/category/add');?>" method="post">
	<div class="pad-10">
		<div class="col-tab">
			<ul class="tabBut cu-li">
				<li id="tab_setting_1" class="on"
					onclick="SwapTab('setting','on','',6,1);"><?php echo L('catgory_basic');?></li>
			...
			<div id="div_setting_1" class="contentList pad-10">
				<table width="100%" class="table_form ">
					<tr>
						...
						<td><input type='radio' name='addtype' value='0' checked
							id="normal_addid"> <?php echo L('normal_add');?>&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='addtype' value='1'
							onclick="$('#catdir_tr').html(' ');$('#normal_add').html(' ');$('#normal_add').css('display','none');$('#batch_add').css('display','');$('#normal_addid').attr('disabled','true');this.disabled='true'"> <?php echo L('batch_add');?></td>
					</tr>
					<tr>
		 ...
		 <?php
			...
			?>
					...
					<tr>
						...
						<td>
		<?php echo Form::select_category('category_content',$parentid,'name="info[parentid]"',L('please_select_parent_category'),0,-1);?>
		</td>
					</tr>

					<tr>
						<th><?php echo L('catname')?>：</th>
						<td><span id="normal_add"><input type="text" name="info[catname]"
        ...
        <?php echo L('batch_add_tips');?>
					...
					<tr id="catdir_tr">
						...
						<td><input type="text" name="info[catdir]" id="catdir"
					...
					<tr>
						...
						<td><textarea name="info[description]" maxlength="255"
	  ...
	  <?php
			...
			?>
					...
					<tr>
						...
						<td><input type='radio' name='info[ismenu]' value='1' checked> <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;
	  ...
	  <input type='radio' name='setting[ishtml]' value='0'
					...
					<tr>
						...
						<td><input type='radio' name='setting[content_ishtml]' value='1'
	...
	?>
	  ...
	  （<?php echo L('create_to_rootdir_tips');?>）</td>
					...
					<tr id="tr_domain" style="display:<?php if(!$setting['ishtml']) echo 'none';?>">
						...
						<td><input type="text" name="info[url]" id="url"
							class="input-text" size="50" value=""></td>
					</tr>
				</table>
			</div>
				...
				<table width="100%" class="table_form ">
					...
					<tr>
						...
						<td><textarea name='setting[meta_keywords]' id='meta_keywords'
								style="width: 90%; height: 40px"><?php echo $setting['meta_keywords'];?></textarea></td>
					</tr>
					<tr>
						<th><?php echo L('meta_description');?></th>
						<td><textarea name='setting[meta_description]'
								id='meta_description' style="width: 90%; height: 50px"><?php echo $setting['meta_description'];?></textarea></td>
					</tr>
				</table>
			</div>
			<div id="div_setting_5" class="contentList pad-10 hidden">
				...
				foreach ( $roles as $roleid => $rolrname ) {
		  		...
		  		<tr>
										...
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?> value="init,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?> value="add,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?> value="edit,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?> value="delete,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?>
											value="listorder,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?> value="push,<?php echo $roleid;?>"></td>
										<td align="center"><input type="checkbox" name="priv_roleid[]"
											<?php echo $disabled;?> value="move,<?php echo $roleid;?>"></td>
									</tr>
			  <?php }?>
					...
					<tr>
						<td colspan=2><hr style="border: 1px dotted #F2F2F2;"></td>
					</tr>
			...
			<div id="div_setting_6" class="contentList pad-10 hidden">
				<table width="100%" class="table_form">
					<tr>
						...
						<td><input name='setting[presentpoint]' type='text' value='1'
							size='5' maxlength='5' style='text-align: center'> <?php echo L('contribute_add_point_tips');?></td>
					</tr>
					<tr>
						<th><?php echo L('default_readpoint');?></th>
						<td><input name='setting[defaultchargepoint]' type='text'
							value='0' size='4' maxlength='4' style='text-align: center'> <select
							name="setting[paytype]"><option value="0"><?php echo L('readpoint');?></option>
								<option value="1"><?php echo L('money');?></option></select> <?php echo L('readpoint_tips');?></td>
					</tr>
					<tr>
						<th><?php echo L('repeatchargedays');?></th>
						<td><input name='setting[repeatchargedays]' type='text' value='1'
        ...
        <font color="red"><?php echo L('repeat_tips2');?></font></td>
			...
			<input name="catid" type="hidden" value="<?php echo $catid;?>"> <input
				name="dosubmit" type="submit" value="<?php echo L('submit')?>"
				class="button">
...
<!--
	...
	function change_tpl(modelid) {
		if(modelid) {
			$.getJSON('?app=admin&controller=category&action=public_change_tpl&modelid='+modelid, function(data){$('#template_list').val(data.template_list);$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
		}
	}
	function load_file_list(id) {
		if(id=='') return false;
		$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&style='+id+'&catid=<?php echo $parentid?>', function(data){$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
	}
