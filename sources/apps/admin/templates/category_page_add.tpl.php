		...
		<div class="col-tab">
			<ul class="tabBut cu-li">
				<li id="tab_setting_1" class="on"
					onclick="SwapTab('setting','on','',5,1);"><?php echo L('catgory_basic');?></li>
			...
			<div id="div_setting_3" class="contentList pad-10 hidden">
				<table width="100%" class="table_form ">
					<tr>
						...
						<td><?php echo Form::select($template_list, $setting['template_list'], 'name="setting[template_list]" id="template_list" onchange="load_file_list(this.value)"', L('please_select'))?></td>
					</tr>
					<tr>
						<th width="200"><?php echo L('page_templates')?>：</th>
						<td id="page_template"></td>
					</tr>
				</table>
			</div>
					...
					<tr>
						...
						<td>
							<table width="100%" class="table-list">
								...
								<tbody>
					<?php $roles = S('common/role');foreach($roles as $roleid=> $rolrname) {$disabled = $roleid==1 ? 'disabled' : '';?>
		  			<tr>
			...
			<input name="catid" type="hidden" value="<?php echo $catid;?>"> <input
				name="type" type="hidden" value="<?php echo $type;?>"> <input
...
<!--
	...
	function load_file_list(id) {
		if(id=='') return false;
		$.getJSON('?app=admin&controller=category&action=public_tpl_file_list&style='+id+'&catid=<?php echo $parentid?>&type=1', function(data){$('#page_template').html(data.page_template);});
	}
