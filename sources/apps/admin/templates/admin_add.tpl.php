...
$(function(){
	...
	$("#username")
		...
		.ajaxValidator({
	    	type : "get",
			url : "?app=admin&controller=admin&action=public_checkname_ajx",
			data :"",
			datatype : "html",
			async:'false',
...
<div class="pad_10">
  <div class="common-form">
    <form name="myform" action="?app=admin&controller=admin&action=add"
			method="post" id="myform">
      <table width="100%" class="table_form contentWrap">
        <tr>
          <td width="80"><?php echo L('username')?></td>
          <td><input type="test" name="info[username]" class="input-text"
						id="username">
            </input></td>
        </tr>
        <tr>
          <td><?php echo L('password')?></td>
          <td><input type="password" name="info[password]" class="input-text"
						id="password" value="">
            </input></td>
        </tr>
        <tr>
          ...
          <td><input type="password" name="info[pwdconfirm]"
						class="input-text" id="pwdconfirm" value="">
            </input></td>
        </tr>
        <tr>
          <td><?php echo L('email')?></td>
          <td><input type="text" name="info[email]" value=""
						class="input-text" id="email" size="30">
            </input></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td><input type="text" name="info[realname]" value=""
						class="input-text" id="realname">
            </input></td>
        </tr>
        <tr>
          <td><?php echo L('userinrole')?></td>
          <td><select name="info[roleid]">
              <?php foreach($roles as $role){ if($_SESSION ['roleid'] != 1 && $role['roleid']==1) continue;?>
