...
<!--
	$(function(){
		...
		$("#name")
			.formValidator({
				onshow:"<?php echo L('input_dbsource_name')?>",
				onfocus:"<?php echo L('input_dbsource_name')?>"
			})
			.inputValidator({
				min:1,
			...
			.regexValidator({
				...
				datatype:"enum",
				param:'i',
				onerror:"<?php echo L('data_source_of_the_letters_and_figures')?>"
			})
			.ajaxValidator({
				type : "get",
				url : "?app=dbsource&controller=dbsource_admin&action=public_name",
				data :"",
				datatype : "html",
				async:'false',
...
<div class="pad-10">
<form action="?app=dbsource&controller=dbsource_admin&action=add" method="post" id="myform">
<div>
<fieldset>
...
<!--
	function test_connect() {
		$.get('?app=dbsource&controller=dbsource_admin&action=public_test_connect', {host:$('#host').val(),username:$('#username').val(), password:$('#password').val(), port:$('#port').val()}, function(data){if(data==1){alert('<?php echo L('connect_success')?>')}else{alert('<?php echo L('connect_failed')?>')}});
}
//-->
</script>
</body>
