...
<fieldset>
	<legend><?php echo L('one_way')?></legend>
    <?php echo L('js_code')?><font color='red'><?php echo L('this_way_stat_show')?></font><br />
<input name="jscode1" id="jscode1" value='<script language="javascript" src="{SITE_URL}index.php?app=poster&controller=index&action=show_poster&id=<?php echo $r['spaceid']?>"></script>' style="width:410px"> <input type="button" onclick="$('#jscode1').select();document.execCommand('Copy');" value="<?php echo L('copy_code')?>" class="button" style="width:114px">
</fieldset>
<div class="bk10"></div>
<fieldset>
	<legend><?php echo L('second_code')?></legend>
    <?php echo L('js_code_html')?><br />
<input name="jscode2" id="jscode2" value='<script language="javascript" src="{SITE_URL}data/<?php echo $r['path']?>"></script>' style="width:410px">
