...
?>
<form method="post" action="?app=poster&controller=poster&action=edit&id=<?php echo $_GET['id']?>&spaceid=<?php echo $info['spaceid']?>" id="myform">
...
$(document).ready(function(){
	...
	$('#name')
		...
		.inputValidator({
			min:6,
			onerror:"<?php echo L('adsname_no_empty')?>"
		})
		.ajaxValidator({
			type:"get",
			url:"?app=poster&controller=poster&action=public_check_poster&id=<?php echo $_GET['id']?>&spaceid=<?php echo $info['spaceid']?>",
			data:"",
			datatype:"html",
			cached:false,
			async:'true',
	...
	$('#type')
		...
		.inputValidator({
			min:1,
			onerror: "<?php echo L('choose_ads_type')?>"
		});
	$('#startdate')
		.formValidator({
	...
	$('#title')
		...
		.inputValidator({
			min:1,
			onerror:'<?php echo L('no_link_content')?>'
		});
	<?php } elseif($sinfo['type']=='code') {?>
	$('#code')
		.formValidator({
			onshow:"<?php echo L('input_code')?>",
...
function preview(iframeWin,returnid){
