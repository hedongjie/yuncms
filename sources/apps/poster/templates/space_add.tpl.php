...
?>
<form method="post" action="?app=poster&controller=space&action=add" name="myform" id="myform">
...
$(document).ready(function(){
	 $.formValidator.initConfig({
		 ...
		 onerror:function(msg,obj){
			 window.top.art.dialog({content:msg,lock:true,width:'220',height:'70'},
	...
	$("#name")
		...
		.inputValidator({
			min:6,
			onerror:"<?php echo L('spacename_illegality')?>"
		})
		.ajaxValidator({
			type:"get",
			url:"?app=poster&controller=space&action=public_check_space&spaceid=<?php echo $_GET['spaceid']?>",
			data:"",
			datatype:"html",
			cached:false,
			async:'true',
