 ...
 *          $
 */
class PosterController extends admin {
	private $db, $s_db;
	...
	public function init() {
		...
		$infos = $this->db->listinfo ( array ('spaceid' => $spaceid ), '`listorder` ASC, `id` DESC', $page );
	...
	public function add() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			if ($id) {
				$this->s_db->update ( array ('items' => '+=1' ), array ('spaceid' => $poster ['spaceid'] ) );
			...
			$sinfo = $this->s_db->get_one ( array ('spaceid' => $spaceid ), 'name, type' );
	...
	public function edit() {
		...
		if (isset ( $_POST ['dosubmit'] )) {
			...
			$setting = $this->check_setting ( $_POST ['setting'], $poster ['type'] );
			$poster ['setting'] = array2string ( $setting );
			$this->db->update ( $poster, array ('id' => $_GET ['id'] ) );
			...
			$sinfo = $this->s_db->get_one ( array ('spaceid' => $info ['spaceid'] ), 'name, type' );
			$setting = $this->get_setting ( $sinfo ['type'] );
			$TYPES = get_types ();
	...
	private function _approval($id = 0) {
		...
		$this->db->update ( array ('disabled' => $_GET ['passed'] ), array ('id' => $id ) );
		return true;
	}
	...
	private function _del($id = 0) {
		$id = intval ( $id );
		if (! $id) return false;
		$r = $this->db->get_one ( array ('id' => $id ), 'spaceid' );
		$this->s_db->update ( array ('items' => '-=1' ), array ('spaceid' => $r ['spaceid'] ) );
	...
	public function public_check_poster() {
		...
		if (isset($_GET ['id'])) {
			$spaceid = intval ( $_GET ['spaceid'] );
			$r = $this->db->get_one ( array ('id' => intval ( $_GET ['id'] ) ) );
			if ($r ['name'] == $_GET ['name']) exit ( '1' );
		}
		$r = $this->db->get_one ( array ('name' => $_GET ['name'] ), 'id' );
		if (isset($r ['id'])) exit ( '0' );
		else exit ( '1' );
	}
}