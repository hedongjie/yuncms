...
<!--
	...
	$("#link_name")
		.formValidator({
			onshow:"<?php echo L("input").L('link_name')?>",
			onfocus:"<?php echo L("input").L('link_name')?>"
		})
		.inputValidator({
			min:1,
		...
		.ajaxValidator({
			type : "get",
			url : "?app=link&controller=link&action=public_name&linkid=<?php echo $linkid;?>",
			data :"",
			datatype : "html",
			async:'false',
	...
	$("#link_url")
		.formValidator({
			onshow:"<?php echo L("input").L('url')?>",
			onfocus:"<?php echo L("input").L('url')?>"
		})
		.inputValidator({
			min:1,
...
<div class="pad_10">
<form action="?app=link&controller=link&action=edit&linkid=<?php echo $linkid; ?>" method="post" name="myform" id="myform">
...
<tr>
		<th></th>
		<td><input type="hidden" name="forward" value="?app=link&controller=link&action=edit"> <input
