 ...
 * @version $Id: IpSource.php 2 2013-01-14 07:14:05Z xutongle $
 */
class IpSource {
	...
	public function __construct() {
		if(@file_exists(FW_PATH.DIRECTORY_SEPARATOR.'resource'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'mini.Dat')) {
			$this->func = 'data_mini';
			$this->fp = @fopen(FW_PATH.DIRECTORY_SEPARATOR.'resource'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'mini.Dat', 'rb');
			$this->offset = unpack('Nlen', fread($this->fp, 4));
			$this->index  = fread($this->fp, $this->offset['len'] - 4);
		} elseif(@file_exists(FW_PATH.DIRECTORY_SEPARATOR.'resource'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'QQWry.Dat')) {
			$this->func = 'data_full';
			$this->fp = @fopen(FW_PATH.DIRECTORY_SEPARATOR.'resource'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'QQWry.Dat', 'rb');
		}
	}
	...
	public function getcitybyapi($ip) {
		...
		$data = $xml->unserialize(@file_get_contents($api_url));
		if (CHARSET == 'gbk') {
		...
		if($data['ip']['result']) {
			...
			$letters = Loader::lib('Pinyin')->output($name,false);//Iconv::gbk_to_pinyin($name);
	...
	public function getcity($ip) {
		...
		$name = CHARSET == 'gbk' ? $localinfo['city'] : iconv('utf-8','gbk',$localinfo['city']);
		$name = str_replace(L('city'),'',$name);
		$letters = Loader::lib('Pinyin')->output($name,false);
		$localinfo['pinyin'] =strtolower(implode('', $letters));
		return $localinfo;
	}
